...
inline Widget Column(ColumnArgs args = {})
{
    ...
    if (!args.children.empty()) intrinsicH += args.spacing * (int)(args.children.size() - 1);
        ...
        if (dbg.enabled && dbg.showColumn) {
            ...
            if (overflowY > 0) {
                ...
                for (int offset = 0; offset < overflowRect.w + overflowRect.h; offset += HATCH_STEP) {
                    int x1 = overflowRect.x + offset, y1 = overflowRect.y;
                    int x2 = overflowRect.x,          y2 = overflowRect.y + offset;
                    if (x1 > overflowRect.x + overflowRect.w) { y1 += x1 - (overflowRect.x + overflowRect.w); x1 = overflowRect.x + overflowRect.w; }
                    if (y2 > overflowRect.y + overflowRect.h) { x2 += y2 - (overflowRect.y + overflowRect.h); y2 = overflowRect.y + overflowRect.h; }
                    SDL_RenderDrawLine(r, x1, y1, x2, y2);
                }
                SDL_SetRenderDrawColor(r, 255, 60, 60, 200);
                const int DASH = 6;
                for (int x = overflowRect.x; x < overflowRect.x + overflowRect.w; x += DASH * 2) {
                    SDL_RenderDrawLine(r, x, overflowRect.y, x + DASH, overflowRect.y);
                    SDL_RenderDrawLine(r, x, overflowRect.y + overflowRect.h, x + DASH, overflowRect.y + overflowRect.h);
                }
                SDL_RenderDrawLine(r, overflowRect.x, overflowRect.y, overflowRect.x, overflowRect.y + overflowRect.h);
                SDL_RenderDrawLine(r, overflowRect.x + overflowRect.w, overflowRect.y, overflowRect.x + overflowRect.w, overflowRect.y + overflowRect.h);
...
inline Widget Column(MainAxisAlignment mainAlign,
                     CrossAxisAlignment crossAlign,
                     int spacing,
