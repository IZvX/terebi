...
inline Widget RoundedBox(Vector2 size, WidgetStyle style, Widget child = {}) 
{
    ...
    Widget w = {"RoundedBox", size, style, {}, {}, 
    [style, shouldAutoSize](SDL_Renderer* renderer, SDL_Rect rect, const WidgetStyle& s,
        ...
        if (s.borderWidth > 0 && s.borderColor.a > 0) {
            SDL_Rect borderRect = {
                rect.x - s.borderWidth,
                rect.y - s.borderWidth,
                rect.w + s.borderWidth * 2,
                rect.h + s.borderWidth * 2
            };
            FillRoundedBoxAA(renderer, borderRect, s.radius + s.borderWidth, s.borderColor);
        }
    ...
    if (shouldAutoSize && !w.children.empty()) {
        ...
        Vector2 padding = style.padding;           // Add your padding field
...
inline Widget Box(Vector2 size, WidgetStyle style, Widget child = {}) 
{
    ...
    [style, shouldAutoSize](SDL_Renderer* renderer, SDL_Rect rect, const WidgetStyle& s,
        ...
        if (s.borderWidth > 0 && s.borderColor.a > 0) {
            ...
            FillBox(renderer, borderRect, s.borderColor);
        }
